...
import {
  ...
  Loader2, ChevronDown, ChevronUp, Sparkles, Clock, Zap, StopCircle
} from "lucide-react";
...
export default function SessionPage() {
  ...
  return (
    <div className="flex h-screen">
      {/* Left sidebar - Tasks */}
      <div className="w-64 shrink-0 border-r border-[var(--card-border)] bg-[var(--card)] p-4 overflow-y-auto hidden md:block">
        <div className="mb-5">
          <Link href="/" className="inline-flex items-center gap-2 rounded-xl px-3 py-2 text-xs text-[var(--muted)] hover:text-[var(--foreground)] hover:bg-[var(--surface-elevated)] cursor-pointer transition-all duration-200">
            <ArrowLeft size={14} />
            返回列表
          </Link>
        </div>
          ...
          {uploadedFiles.length > 0 && (
            <div className="mb-2 flex flex-wrap gap-2">
              {uploadedFiles.map((f, i) => (
                <span key={i} className="inline-flex items-center gap-1.5 rounded-lg border border-[var(--card-border)] bg-[var(--surface-elevated)] px-2.5 py-1 text-xs">
                  <Paperclip size={10} className="text-[var(--muted)]" />
                  {f.name} ({(f.size / 1024).toFixed(1)}KB)
                  <button onClick={() => setUploadedFiles(prev => prev.filter((_, idx) => idx !== i))} className="text-[var(--muted)] hover:text-[var(--danger)] cursor-pointer transition-colors"><X size={12} /></button>
                </span>
              ))}
            </div>
          )}
          <div className="flex items-center gap-2">
            <label className="flex size-10 items-center justify-center rounded-xl border border-[var(--card-border)] bg-[var(--surface-elevated)] text-[var(--muted)] cursor-pointer hover:border-[var(--accent)] hover:text-[var(--foreground)] transition-all" title="上传文件">
              <Paperclip size={16} />
              <input
                type="file"
                multiple
            ...
            <input
              ...
              className="h-10 flex-1 rounded-xl border border-[var(--card-border)] bg-[var(--surface-elevated)] px-4 text-sm outline-none transition-all focus:border-[var(--accent)] focus:shadow-[var(--shadow-glow)]"
            />
            <button
              onClick={sendMessage}
      ...
      <div className="w-80 shrink-0 border-l border-[var(--card-border)] bg-[var(--card)] p-4 overflow-y-auto hidden lg:block">
