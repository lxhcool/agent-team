...
def _build_functional_workspace_html(
    workspace: Workspace,
    stages: List[WorkspaceStage],
    fallback_html: str,
) -> str:
    # Development preview must stay visually aligned with the approved prototype.
    # Do not regenerate a separate UI here; this stage should inherit the prototype
    # and only evolve implementation details in later iterations.
    return fallback_html


def _build_functional_todo_html(workspace: Workspace, stages: List[WorkspaceStage], brief: Dict[str, Any]) -> str:
    app_name = _safe_text(str(brief.get("app_name") or workspace.name or "Todo App"))
    subtitle = _safe_text(str(brief.get("hero_subtitle") or "快速记录、切换状态并保存你的任务。"))
    sample_items = brief.get("sample_items") or ["写完首页文案", "确认移动端按钮", "整理周五发布清单"]
    seed = [
        {"id": "seed-1", "title": str(sample_items[0]), "priority": "high", "done": False, "bucket": "today"},
        {"id": "seed-2", "title": str(sample_items[1] if len(sample_items) > 1 else sample_items[0]), "priority": "medium", "done": False, "bucket": "upcoming"},
        {"id": "seed-3", "title": str(sample_items[2] if len(sample_items) > 2 else sample_items[0]), "priority": "low", "done": True, "bucket": "done"},
    ]
    return f"""<!doctype html>
<html lang="zh-CN">
<head>
  ...
  <style>
    :root {{ --bg:#f7f8fb; --panel:#ffffffe3; --line:#e2e8f0; --text:#0f172a; --muted:#64748b; --accent:#0f766e; --accent-soft:#ccfbf1; --success:#059669; --danger:#dc2626; --shadow:0 18px 40px rgba(15,23,42,.08); }}
    * {{ box-sizing:border-box; }} body {{ margin:0; font-family:Inter,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif; color:var(--text); background:linear-gradient(180deg,#f8fafc 0%,#f3f6fb 100%); }}
    button,input,select {{ font:inherit; }} .shell {{ max-width:1240px; margin:0 auto; padding:28px 20px 40px; }} .topbar {{ display:flex; justify-content:space-between; align-items:center; gap:16px; padding:14px 18px; border-radius:20px; border:1px solid rgba(255,255,255,.65); background:rgba(255,255,255,.72); backdrop-filter:blur(18px); box-shadow:0 12px 28px rgba(15,23,42,.06); }}
    .brand {{ display:flex; align-items:center; gap:12px; }} .logo {{ width:42px; height:42px; border-radius:14px; display:grid; place-items:center; background:#0f172a; color:white; font-weight:800; }} .brand h1 {{ margin:0; font-size:20px; }} .brand p {{ margin:2px 0 0; font-size:13px; color:var(--muted); }}
    .sync {{ display:inline-flex; align-items:center; gap:8px; padding:8px 12px; border-radius:999px; background:#ecfeff; color:#0f766e; font-size:12px; font-weight:700; }}
    .hero {{ display:grid; grid-template-columns:320px minmax(0,1fr); gap:22px; margin-top:22px; }} .card {{ border-radius:22px; border:1px solid rgba(255,255,255,.7); background:var(--panel); backdrop-filter:blur(16px); box-shadow:var(--shadow); }}
    .stats {{ padding:22px; display:grid; gap:14px; }} .stats-grid {{ display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:12px; }} .stat {{ border-radius:18px; background:white; border:1px solid var(--line); padding:14px; }} .stat strong {{ display:block; font-size:24px; margin-top:8px; }} .stat span {{ font-size:12px; color:var(--muted); }}
    .tips {{ border-radius:18px; border:1px dashed #99f6e4; background:#f0fdfa; padding:14px; font-size:13px; color:#115e59; line-height:1.6; }} .board {{ padding:22px; }} .board-head {{ display:flex; justify-content:space-between; align-items:flex-start; gap:16px; margin-bottom:18px; }} .board-head h2 {{ margin:0; font-size:28px; }} .board-head p {{ margin:6px 0 0; color:var(--muted); font-size:14px; }}
    .quick-add {{ display:grid; grid-template-columns:minmax(0,1fr) 140px 140px 104px; gap:12px; padding:16px; border-radius:22px; background:white; border:1px solid var(--line); }} .quick-add input,.quick-add select {{ width:100%; height:46px; border-radius:14px; border:1px solid var(--line); background:#fff; padding:0 14px; outline:none; }} .quick-add button {{ border:0; border-radius:14px; background:#0f172a; color:white; font-weight:700; cursor:pointer; }}
    .toolbar {{ display:flex; flex-wrap:wrap; justify-content:space-between; gap:12px; margin:18px 0; }} .chips,.filters {{ display:flex; flex-wrap:wrap; gap:8px; }} .chip,.filter {{ border:1px solid var(--line); background:rgba(255,255,255,.78); color:var(--muted); border-radius:999px; padding:8px 12px; font-size:13px; cursor:pointer; }} .chip.active,.filter.active {{ background:var(--text); color:white; border-color:var(--text); }}
    .columns {{ display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:14px; }} .column {{ border-radius:20px; background:rgba(255,255,255,.7); border:1px solid rgba(226,232,240,.95); padding:14px; min-height:380px; }} .column-head {{ display:flex; justify-content:space-between; align-items:center; margin-bottom:12px; }} .column-head h3 {{ margin:0; font-size:15px; }} .pill {{ border-radius:999px; padding:4px 8px; font-size:11px; font-weight:700; background:#f1f5f9; color:#334155; }}
    .task-list {{ display:grid; gap:10px; }} .task {{ border-radius:18px; border:1px solid var(--line); background:white; padding:14px; display:grid; gap:10px; }} .task.done {{ opacity:.72; }} .task-top {{ display:flex; align-items:flex-start; gap:10px; }} .task-toggle {{ width:20px; height:20px; margin-top:2px; border-radius:999px; border:2px solid #bfdbfe; cursor:pointer; flex:none; display:inline-grid; place-items:center; color:white; background:transparent; }} .task.done .task-toggle {{ background:var(--success); border-color:var(--success); }}
    .task-title {{ font-weight:700; line-height:1.5; }} .task.done .task-title {{ text-decoration:line-through; color:var(--muted); }} .task-meta {{ display:flex; flex-wrap:wrap; gap:8px; font-size:12px; color:var(--muted); }} .badge {{ border-radius:999px; padding:4px 8px; font-weight:700; }} .badge.high {{ background:#fee2e2; color:#b91c1c; }} .badge.medium {{ background:#fef3c7; color:#92400e; }} .badge.low {{ background:#dcfce7; color:#166534; }}
    .task-actions {{ display:flex; justify-content:space-between; align-items:center; gap:10px; }} .task-actions button {{ border:0; background:none; color:var(--muted); cursor:pointer; padding:0; }} .empty {{ border:1px dashed var(--line); border-radius:18px; padding:18px; text-align:center; font-size:13px; color:var(--muted); }} .footer {{ margin-top:20px; display:flex; justify-content:space-between; gap:12px; font-size:12px; color:var(--muted); }}
    @media (max-width:1024px) {{ .hero,.columns {{ grid-template-columns:1fr; }} }} @media (max-width:720px) {{ .shell {{ padding:18px 14px 32px; }} .quick-add {{ grid-template-columns:1fr; }} .board-head h2 {{ font-size:24px; }} .footer {{ flex-direction:column; }} }}
  </style>
</head>
<body><div class="shell"><header class="topbar"><div class="brand"><div class="logo">✓</div><div><h1>{app_name}</h1><p>{subtitle}</p></div></div><div class="sync">本地自动保存</div></header>
<section class="hero"><aside class="card stats"><div><strong style="font-size:18px;">今天的节奏</strong><p style="margin:8px 0 0;color:var(--muted);font-size:13px;line-height:1.6;">新增、完成、删除、筛选和归档都已经可用，数据会保存在当前浏览器。</p></div><div class="stats-grid"><div class="stat"><span>全部任务</span><strong id="totalCount">0</strong></div><div class="stat"><span>已完成</span><strong id="doneCount">0</strong></div><div class="stat"><span>进行中</span><strong id="todoCount">0</strong></div><div class="stat"><span>完成率</span><strong id="progressCount">0%</strong></div></div><div class="tips">可以直接录入真实任务。刷新后不会丢失，适合先把最小可用版本跑通。</div></aside>
<main class="card board"><div class="board-head"><div><h2>任务面板</h2><p>先把事情收进来，再按状态处理。</p></div></div><div class="quick-add"><input id="taskInput" type="text" placeholder="输入一个任务，例如：整理博客首页文章列表" /><select id="taskPriority"><option value="medium">中优先级</option><option value="high">高优先级</option><option value="low">低优先级</option></select><select id="taskBucket"><option value="today">今天</option><option value="upcoming">即将</option><option value="done">已完成</option></select><button id="addTaskBtn" type="button">添加任务</button></div><div class="toolbar"><div class="chips"><button class="chip active" type="button" data-filter="all">全部</button><button class="chip" type="button" data-filter="active">未完成</button><button class="chip" type="button" data-filter="done">已完成</button></div><div class="filters"><button class="filter active" type="button" data-priority="all">全部优先级</button><button class="filter" type="button" data-priority="high">高</button><button class="filter" type="button" data-priority="medium">中</button><button class="filter" type="button" data-priority="low">低</button></div></div><div class="columns"><section class="column"><div class="column-head"><h3>今天</h3><span class="pill" id="todayBadge">0</span></div><div class="task-list" id="todayList"></div></section><section class="column"><div class="column-head"><h3>即将</h3><span class="pill" id="upcomingBadge">0</span></div><div class="task-list" id="upcomingList"></div></section><section class="column"><div class="column-head"><h3>已完成</h3><span class="pill" id="doneBadge">0</span></div><div class="task-list" id="doneList"></div></section></div><div class="footer"><span>已具备最基本任务管理交互，不再是静态展示页。</span><button id="resetTasksBtn" type="button" style="border:0;background:none;color:var(--danger);cursor:pointer;padding:0;">重置演示数据</button></div></main></section></div>
<script>
const storageKey="agent-team-functional-todo-v1"; const seedTasks={json.dumps(seed, ensure_ascii=False)}; let state={{tasks:[],statusFilter:"all",priorityFilter:"all"}}; const els={{taskInput:document.getElementById("taskInput"),taskPriority:document.getElementById("taskPriority"),taskBucket:document.getElementById("taskBucket"),addTaskBtn:document.getElementById("addTaskBtn"),resetTasksBtn:document.getElementById("resetTasksBtn"),totalCount:document.getElementById("totalCount"),doneCount:document.getElementById("doneCount"),todoCount:document.getElementById("todoCount"),progressCount:document.getElementById("progressCount"),todayList:document.getElementById("todayList"),upcomingList:document.getElementById("upcomingList"),doneList:document.getElementById("doneList"),todayBadge:document.getElementById("todayBadge"),upcomingBadge:document.getElementById("upcomingBadge"),doneBadge:document.getElementById("doneBadge")}};
function load(){{try{{const raw=localStorage.getItem(storageKey); if(!raw) throw new Error("empty"); const parsed=JSON.parse(raw); state.tasks=Array.isArray(parsed.tasks)?parsed.tasks:[...seedTasks];}}catch{{state.tasks=[...seedTasks];}}}} function save(){{localStorage.setItem(storageKey, JSON.stringify({{tasks:state.tasks}}));}} function makeId(){{return "task-"+Date.now()+"-"+Math.random().toString(16).slice(2,8);}} function matchesFilters(task){{const statusOk=state.statusFilter==="all"||(state.statusFilter==="done"&&task.done)||(state.statusFilter==="active"&&!task.done); const priorityOk=state.priorityFilter==="all"||task.priority===state.priorityFilter; return statusOk&&priorityOk;}}
function toggleTask(id){{state.tasks=state.tasks.map(task=>task.id!==id?task:{{...task,done:!task.done,bucket:!task.done?"done":(task.bucket==="done"?"today":task.bucket)}}); save(); render();}} function removeTask(id){{state.tasks=state.tasks.filter(task=>task.id!==id); save(); render();}} function addTask(){{const title=els.taskInput.value.trim(); if(!title){{els.taskInput.focus(); return;}} const bucket=els.taskBucket.value; state.tasks.unshift({{id:makeId(),title,priority:els.taskPriority.value,bucket,done:bucket==="done"}}); els.taskInput.value=""; els.taskBucket.value="today"; els.taskPriority.value="medium"; save(); render(); els.taskInput.focus();}}
function taskCard(task){{const item=document.createElement("article"); item.className="task"+(task.done?" done":""); item.innerHTML=`<div class="task-top"><button class="task-toggle" type="button">${{task.done?"✓":""}}</button><div style="min-width:0;flex:1;"><div class="task-title"></div><div class="task-meta"><span class="badge ${{task.priority}}">${{task.priority==="high"?"高优先级":task.priority==="medium"?"中优先级":"低优先级"}}</span><span>${{task.bucket==="today"?"今天":task.bucket==="upcoming"?"即将":"已完成"}}</span></div></div></div><div class="task-actions"><span style="font-size:12px;color:var(--muted);">${{task.done?"已完成，可继续删除":"点击左侧圆点即可完成"}}</span><button type="button">删除</button></div>`; item.querySelector(".task-title").textContent=task.title; item.querySelector(".task-toggle").addEventListener("click",()=>toggleTask(task.id)); item.querySelector(".task-actions button").addEventListener("click",()=>removeTask(task.id)); return item;}}
function renderColumn(target,tasks,emptyText){{target.innerHTML=""; if(!tasks.length){{const empty=document.createElement("div"); empty.className="empty"; empty.textContent=emptyText; target.appendChild(empty); return;}} tasks.forEach(task=>target.appendChild(taskCard(task)));}}
function render(){{const filtered=state.tasks.filter(matchesFilters); const today=filtered.filter(task=>task.bucket==="today"&&!task.done); const upcoming=filtered.filter(task=>task.bucket==="upcoming"&&!task.done); const done=filtered.filter(task=>task.done||task.bucket==="done"); renderColumn(els.todayList,today,"今天还没有任务，先添加一条。"); renderColumn(els.upcomingList,upcoming,"后续任务会显示在这里。"); renderColumn(els.doneList,done,"完成后会自动进入这里。"); const total=state.tasks.length; const doneCount=state.tasks.filter(task=>task.done).length; const todoCount=total-doneCount; els.totalCount.textContent=String(total); els.doneCount.textContent=String(doneCount); els.todoCount.textContent=String(todoCount); els.progressCount.textContent=total?Math.round(doneCount/total*100)+"%":"0%"; els.todayBadge.textContent=String(today.length); els.upcomingBadge.textContent=String(upcoming.length); els.doneBadge.textContent=String(done.length);}}
document.querySelectorAll("[data-filter]").forEach(btn=>btn.addEventListener("click",()=>{{document.querySelectorAll("[data-filter]").forEach(item=>item.classList.remove("active")); btn.classList.add("active"); state.statusFilter=btn.dataset.filter||"all"; render();}})); document.querySelectorAll("[data-priority]").forEach(btn=>btn.addEventListener("click",()=>{{document.querySelectorAll("[data-priority]").forEach(item=>item.classList.remove("active")); btn.classList.add("active"); state.priorityFilter=btn.dataset.priority||"all"; render();}})); els.addTaskBtn.addEventListener("click",addTask); els.taskInput.addEventListener("keydown",event=>{{if(event.key==="Enter") addTask();}}); els.resetTasksBtn.addEventListener("click",()=>{{state.tasks=[...seedTasks]; save(); render();}}); load(); render();
</script></body></html>"""


def _build_functional_blog_html(workspace: Workspace, stages: List[WorkspaceStage], brief: Dict[str, Any]) -> str:
    app_name = _safe_text(str(brief.get("app_name") or workspace.name or "个人博客"))
    subtitle = _safe_text(str(brief.get("hero_subtitle") or "把长期想法整理成可读、可筛选、可继续浏览的内容站点。"))
    sample_items = brief.get("sample_items") or ["为什么要长期写作", "重新整理首页结构", "用内容驱动个人品牌"]
    sample_descriptions = brief.get("sample_descriptions") or ["记录长期观察。", "让文章更容易被浏览。", "让作品和表达形成完整入口。"]
    posts = [
        {"id":"post-1","category":"产品","title":str(sample_items[0]),"excerpt":str(sample_descriptions[0]),"body":"这篇文章讨论为什么个人博客不该只是摆设页面，而要真正承接内容浏览、分类查找和持续更新。第一版先把阅读体验、文章列表和归档逻辑做扎实。","date":"2026-05-02","tag":"长期写作","readTime":"6 分钟阅读"},
        {"id":"post-2","category":"设计","title":str(sample_items[1] if len(sample_items)>1 else sample_items[0]),"excerpt":str(sample_descriptions[1] if len(sample_descriptions)>1 else sample_descriptions[0]),"body":"这篇文章记录首页信息层级的调整过程，重点是让访客进入站点后立刻看到作者、最新内容和继续阅读入口，而不是先看到一块空泛介绍。","date":"2026-04-28","tag":"界面结构","readTime":"5 分钟阅读"},
        {"id":"post-3","category":"随笔","title":str(sample_items[2] if len(sample_items)>2 else sample_items[0]),"excerpt":str(sample_descriptions[2] if len(sample_descriptions)>2 else sample_descriptions[0]),"body":"这篇文章更偏个人表达，讨论为什么内容网站需要把文章、项目和关于页放在同一个阅读语境中，而不是拆成互不相关的静态页面。","date":"2026-04-21","tag":"个人品牌","readTime":"4 分钟阅读"},
    ]
    return f"""<!doctype html>
<html lang="zh-CN"><head><meta charset="utf-8" /><meta name="viewport" content="width=device-width, initial-scale=1" /><title>{app_name}</title>
<style>
:root{{--bg:#f8fafc;--surface:rgba(255,255,255,.92);--line:#e2e8f0;--text:#0f172a;--muted:#64748b;--accent:#0f766e;--accent-soft:#ccfbf1;--shadow:0 18px 40px rgba(15,23,42,.08);}}*{{box-sizing:border-box;}}body{{margin:0;font-family:Inter,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:var(--text);background:linear-gradient(180deg,#fbfdff 0%,#f4f7fb 100%);}}button,input{{font:inherit;}} .shell{{max-width:1240px;margin:0 auto;padding:28px 20px 48px;}} .topbar{{display:flex;justify-content:space-between;align-items:center;gap:16px;padding:14px 18px;border-radius:20px;border:1px solid rgba(255,255,255,.65);background:rgba(255,255,255,.86);backdrop-filter:blur(18px);box-shadow:0 12px 28px rgba(15,23,42,.05);}} .brand h1{{margin:0;font-size:22px;}} .brand p{{margin:4px 0 0;font-size:13px;color:var(--muted);}} .nav{{display:flex;gap:10px;flex-wrap:wrap;}} .nav button{{border:1px solid var(--line);background:white;border-radius:999px;padding:8px 12px;color:var(--muted);cursor:pointer;}}
.hero{{margin-top:22px;padding:26px;border-radius:28px;border:1px solid rgba(255,255,255,.75);background:var(--surface);box-shadow:var(--shadow);display:grid;gap:18px;}} .hero h2{{margin:0;font-size:34px;max-width:780px;line-height:1.18;}} .hero p{{margin:0;max-width:760px;color:var(--muted);line-height:1.8;}} .hero-actions{{display:flex;flex-wrap:wrap;gap:10px;}} .hero-actions button{{border:0;border-radius:14px;padding:12px 16px;font-weight:700;cursor:pointer;}} .hero-actions .primary{{background:var(--text);color:white;}} .hero-actions .secondary{{background:#ecfeff;color:#115e59;}}
.layout{{display:grid;grid-template-columns:minmax(0,360px) minmax(0,1fr);gap:20px;margin-top:22px;}} .panel{{border-radius:24px;border:1px solid rgba(255,255,255,.75);background:var(--surface);box-shadow:var(--shadow);overflow:hidden;}} .panel-head{{padding:18px 20px;border-bottom:1px solid var(--line);}} .panel-head h3{{margin:0;font-size:16px;}} .panel-head p{{margin:6px 0 0;font-size:13px;color:var(--muted);}}
.controls{{padding:16px 20px 0;display:grid;gap:12px;}} .search{{width:100%;height:44px;border-radius:14px;border:1px solid var(--line);background:white;padding:0 14px;outline:none;}} .tags{{display:flex;flex-wrap:wrap;gap:8px;}} .tag{{border:1px solid var(--line);background:rgba(255,255,255,.8);color:var(--muted);border-radius:999px;padding:8px 12px;font-size:12px;cursor:pointer;}} .tag.active{{background:var(--text);border-color:var(--text);color:white;}} .list{{padding:16px 20px 20px;display:grid;gap:12px;}}
.post-card{{width:100%;text-align:left;border:1px solid var(--line);background:white;border-radius:18px;padding:16px;cursor:pointer;display:grid;gap:8px;}} .post-card.active{{border-color:#99f6e4;box-shadow:0 0 0 4px rgba(20,184,166,.10);}} .post-card strong{{font-size:16px;line-height:1.5;}} .meta{{display:flex;flex-wrap:wrap;gap:8px;font-size:12px;color:var(--muted);}} .kicker{{font-size:11px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:var(--accent);}}
.reader{{padding:22px;display:grid;gap:18px;}} .reader h2{{margin:0;font-size:30px;line-height:1.25;}} .reader p{{margin:0;color:#334155;line-height:1.9;}} .reader-meta{{display:flex;flex-wrap:wrap;gap:10px;font-size:12px;color:var(--muted);}} .reader-actions{{display:flex;flex-wrap:wrap;gap:10px;}} .reader-actions button{{border:0;border-radius:14px;padding:10px 14px;font-weight:700;cursor:pointer;}} .reader-actions .primary{{background:var(--text);color:white;}} .reader-actions .secondary{{background:#ecfeff;color:#115e59;}} .reader-note{{border-radius:18px;background:#f8fafc;border:1px dashed #99f6e4;padding:14px;font-size:13px;color:#115e59;}} .outline{{border-top:1px solid var(--line);padding-top:16px;display:grid;gap:10px;}} .outline span{{font-size:13px;color:var(--muted);}} .empty{{padding:18px;border:1px dashed var(--line);border-radius:18px;text-align:center;color:var(--muted);font-size:13px;}} @media (max-width:980px){{.layout{{grid-template-columns:1fr;}}}} @media (max-width:720px){{.shell{{padding:18px 14px 32px;}} .hero h2{{font-size:28px;}} .reader h2{{font-size:24px;}}}}
</style></head>
<body><div class="shell"><header class="topbar"><div class="brand"><h1>{app_name}</h1><p>{subtitle}</p></div><nav class="nav"><button type="button" id="jumpLatest">最新文章</button><button type="button" id="jumpArchive">归档筛选</button></nav></header>
<section class="hero"><div class="kicker">内容型网站</div><h2>这不是静态展示页，而是已经能浏览、筛选、切换阅读内容的博客首页。</h2><p>左侧可以搜索和筛选文章，右侧直接切换阅读内容。第一版先把内容浏览和阅读体验做实，再继续扩展后台、上传和发布流。</p><div class="hero-actions"><button class="primary" type="button" id="focusReader">阅读最新文章</button><button class="secondary" type="button" id="focusFilter">按分类查看</button></div></section>
<section class="layout"><aside class="panel" id="archivePanel"><div class="panel-head"><h3>文章列表</h3><p>支持搜索、分类筛选和切换阅读。</p></div><div class="controls"><input class="search" id="searchInput" type="text" placeholder="搜索文章标题或摘要" /><div class="tags"><button class="tag active" type="button" data-category="全部">全部</button><button class="tag" type="button" data-category="产品">产品</button><button class="tag" type="button" data-category="设计">设计</button><button class="tag" type="button" data-category="随笔">随笔</button></div></div><div class="list" id="postList"></div></aside>
<article class="panel reader" id="readerPanel"><div class="kicker" id="readerCategory">产品</div><h2 id="readerTitle"></h2><div class="reader-meta"><span id="readerDate"></span><span id="readerReadTime"></span><span id="readerTag"></span></div><p id="readerBody"></p><div class="reader-actions"><button class="primary" type="button" id="nextPost">下一篇</button><button class="secondary" type="button" id="toggleSummary">切换摘要</button></div><div class="reader-note" id="readerNote">这里已经是可切换的阅读态，不再只是静态介绍。</div><div class="outline"><strong>阅读路径</strong><span>01. 首页看到最新文章与作者定位</span><span>02. 左侧筛选并切换不同类型的内容</span><span>03. 右侧继续阅读，并回到归档继续浏览</span></div></article></section></div>
<script>
const posts={json.dumps(posts, ensure_ascii=False)}; let activeCategory="全部"; let activeSearch=""; let activeId=posts[0]?.id||null; let showExcerpt=false; const postList=document.getElementById("postList"); const searchInput=document.getElementById("searchInput"); const readerTitle=document.getElementById("readerTitle"); const readerBody=document.getElementById("readerBody"); const readerCategory=document.getElementById("readerCategory"); const readerDate=document.getElementById("readerDate"); const readerReadTime=document.getElementById("readerReadTime"); const readerTag=document.getElementById("readerTag"); const readerNote=document.getElementById("readerNote");
function filteredPosts(){{return posts.filter(post=>{{const categoryOk=activeCategory==="全部"||post.category===activeCategory; const searchOk=!activeSearch||(post.title+" "+post.excerpt+" "+post.body).toLowerCase().includes(activeSearch.toLowerCase()); return categoryOk&&searchOk;}});}} function ensureActive(items){{if(!items.some(post=>post.id===activeId)) activeId=items[0]?.id||null;}}
function renderReader(post){{if(!post){{readerTitle.textContent="没有匹配文章"; readerBody.textContent="调整一下分类或搜索词，再继续查看。"; readerCategory.textContent="空结果"; readerDate.textContent=""; readerReadTime.textContent=""; readerTag.textContent=""; readerNote.textContent="当前筛选条件下没有文章。"; return;}} readerTitle.textContent=post.title; readerBody.textContent=showExcerpt?post.excerpt:post.body; readerCategory.textContent=post.category; readerDate.textContent=post.date; readerReadTime.textContent=post.readTime; readerTag.textContent="# "+post.tag; readerNote.textContent=showExcerpt?"当前显示摘要状态，适合快速判断文章方向。":"当前显示正文状态，适合确认阅读区的内容密度和节奏。";}}
function renderList(){{const items=filteredPosts(); ensureActive(items); postList.innerHTML=""; if(!items.length){{const empty=document.createElement("div"); empty.className="empty"; empty.textContent="没有找到匹配文章，换个关键词试试。"; postList.appendChild(empty); renderReader(null); return;}} items.forEach(post=>{{const button=document.createElement("button"); button.type="button"; button.className="post-card"+(post.id===activeId?" active":""); button.innerHTML=`<div class="kicker">${{post.category}}</div><strong></strong><div style="font-size:13px;color:var(--muted);line-height:1.7;"></div><div class="meta"><span>${{post.date}}</span><span>${{post.readTime}}</span><span># ${{post.tag}}</span></div>`; button.querySelector("strong").textContent=post.title; button.querySelector("div:nth-of-type(2)").textContent=post.excerpt; button.addEventListener("click",()=>{{activeId=post.id; showExcerpt=false; renderList();}}); postList.appendChild(button);}}); renderReader(items.find(post=>post.id===activeId)||items[0]);}}
document.querySelectorAll("[data-category]").forEach(button=>button.addEventListener("click",()=>{{document.querySelectorAll("[data-category]").forEach(item=>item.classList.remove("active")); button.classList.add("active"); activeCategory=button.dataset.category||"全部"; renderList();}})); searchInput.addEventListener("input",event=>{{activeSearch=event.target.value||""; renderList();}}); document.getElementById("nextPost").addEventListener("click",()=>{{const items=filteredPosts(); if(!items.length) return; const currentIndex=items.findIndex(item=>item.id===activeId); activeId=items[(currentIndex+1)%items.length].id; showExcerpt=false; renderList();}}); document.getElementById("toggleSummary").addEventListener("click",()=>{{showExcerpt=!showExcerpt; const items=filteredPosts(); renderReader(items.find(post=>post.id===activeId)||items[0]||null);}}); document.getElementById("focusReader").addEventListener("click",()=>document.getElementById("readerPanel").scrollIntoView({{behavior:"smooth",block:"start"}})); document.getElementById("focusFilter").addEventListener("click",()=>document.getElementById("archivePanel").scrollIntoView({{behavior:"smooth",block:"start"}})); document.getElementById("jumpLatest").addEventListener("click",()=>{{activeCategory="全部"; document.querySelectorAll("[data-category]").forEach(item=>item.classList.remove("active")); document.querySelector('[data-category="全部"]')?.classList.add("active"); renderList();}}); document.getElementById("jumpArchive").addEventListener("click",()=>document.getElementById("archivePanel").scrollIntoView({{behavior:"smooth",block:"start"}})); renderList();
</script></body></html>"""
...
async def _generate_workspace_development_stage(
    ...
    prototype_bytes = source_path.read_bytes()
    prototype_html = prototype_bytes.decode("utf-8")
    preview_html = _build_functional_workspace_html(workspace, stages, prototype_html)
    ...
    recommendation.update({
        "source": "development_generator_v2",
        "agent_name": _workspace_stage_agent_name(WorkspaceStageKey.DEVELOPMENT),
        "summary": "开发阶段已生成可操作预览，而不再只是静态原型搬运。",
        "recommended_action": "先直接操作这个版本，确认基础交互和页面质量；如果方向正确，再进入验收阶段。",
        "focus": ["基础交互", "可操作预览", "当前交付边界", "是否继续迭代"],
        "execution_session_id": exec_session.id,
        "project_path": exec_session.project_path,
