...
def stage_responsibility_contract(stage_key: WorkspaceStageKey) -> Dict[str, Any]:
    contract_map: Dict[WorkspaceStageKey, Dict[str, Any]] = {
        WorkspaceStageKey.REQUIREMENTS: {
            "chat_scope": "确认产品定位与最基本的使用方式。",
            "chat_focus": "产品定义、基本使用方式、会影响后续方向的前提",
            "chat_avoid": [
                "功能拆分、页面结构、管理结构",
                "规则细则、交互字段、运营口径",
                "技术实现、数据设计、交付方式",
            ],
            "ask_rule": "只有缺少会改变这一阶段判断的信息时，才补 1 个问题。",
...
8. 如果上一轮助手已经把当前阶段的理解、判断或方案整理得很清楚，而用户这轮只是表示“没有补充了”“可以了”“就这样”“继续下一阶段”这类收束信号，只要内容本身足够，就应该返回 true，不要再多走一轮聊天。
